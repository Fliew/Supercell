...
class FLanguage implements FLanguageInterface
{
    /**
     * @access  private
     * @var     array
    ...
    {
        // Load Configs
        $language_config = new FConfig('language');
    ...
    private static function cache($path, $language, $ttl)
    {
        ...
        {
            throw new FErrors('Could not open file');
        }
        
        if (fwrite($handle, $content) === false)
        {
            throw new FErrors('Could not write to file');
        }
        ...
        if ($files = opendir(SERVER_PATH . 'application/public/languages/' . $language . '/'))
        {
            ...
            else
            {
                ...
                for ($i = 0; $i < $size; $i++)
                {
                    ...
                    if (!is_array($$key[$i]))
                    {
                        throw new FErrors('The language file\'s (' . $array[$key[$i]] . ') variable (' . $key[$i] . ') does not have an array.');
                    }
