 ...
 * 
 * @subpackage  FDatabase
 */

class FDatabase implements FDatabaseInterface
{
    /**
     * @access  private
     * @var     string
    ...
    private static function call($name, $params)
    {
        ...
        {
            // Loads config driver
            $config = new FConfig('database');
            
            // Sets the driver for later use
            self::$driver = $config->setting('driver');
            
            require_once(SERVER_PATH . 'system/libraries/FDatabase/drivers/' . self::$driver . '.php');
        }
        else
        {
